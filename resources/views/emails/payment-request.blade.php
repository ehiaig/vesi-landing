...
<body style="background-color: #222533; padding: 20px; font-family: font-size: 14px; line-height: 1.43; font-family: &quot;Helvetica Neue&quot;, &quot;Segoe UI&quot;, Helvetica, Arial, sans-serif;">
	...
	<div style="max-width: 600px; margin: 0px auto; background-color: #fff; box-shadow: 0px 20px 50px rgba(0,0,0,0.05);">
		...
		<div style="padding: 60px 70px; border-top: 1px solid rgba(0,0,0,0.05);">
			<h2 style="margin-top: 0px;">Your Invoice Was Successfully Created!</h2>
			<div style="color: #636363; font-size: 14px;">
				<p>Dear {{$invoice->sender->firstname}}</p>
				<p>Your payment request for invoice  {{$invoice->offline_ref_code}} of {{$invoice->amount}} has been sent to {{$invoice->recipient->firstname}} for pre-authorization. You will be notified once {{$invoice->recipient->firstname}} pre-authorizes the payment, so that you can fulfill the order and receive payment on completion. 
				</p>
			</div>
			<a href="{{ route('invoice.show.index') }}" style="padding: 8px 20px; background-color: #4B72FA; color: #fff; font-weight: bolder; font-size: 16px; display: inline-block; margin: 20px 0px; margin-right: 20px; text-decoration: none;">View Dashboard</a>
		</div>
			<div style="background-color: #F5F5F5; padding: 40px; text-align: center;">
				...
				<!-- <div style="margin-bottom: 20px;">
					...
					<a href="#" style="text-decoration: underline; font-size: 14px; letter-spacing: 1px; margin: 0px 15px; color: #261D1D;">U</a>
				</div> -->
				<div style="color: #A5A5A5; font-size: 12px; margin-bottom: 20px; padding: 0px 50px;">
					You are receiving this email because you signed up for Vesicash Escrow Services
				</div>
